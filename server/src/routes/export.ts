...
router.get("/:novelId/full", async (req, res, next) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent(novel.title + "_全文.txt")}`);
    res.send(txt);
  } catch (error) {
    next(error);
  }
...
router.get("/:novelId/outline", async (req, res, next) => {
  try {
    ...
    const txt = lines.join("\n");
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent(novel.title + "_大纲.txt")}`);
    res.send(txt);
  } catch (error) {
    next(error);
  }
...
router.get("/:novelId/characters", async (req, res, next) => {
  try {
    ...
    const txt = lines.join("\n");
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent(novel.title + "_人物.txt")}`);
    res.send(txt);
  } catch (error) {
    next(error);
  }
...
router.get("/:novelId/worldview", async (req, res, next) => {
  try {
    ...
    const txt = lines.join("\n");
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent(novel.title + "_世界观.txt")}`);
    res.send(txt);
  } catch (error) {
    next(error);
  }
