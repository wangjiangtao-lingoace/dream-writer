...
│  └── 通用知识库管理                                           │
├─────────────────────────────────────────────────────────────┤
│  API 层 (Express 5) — 25 个路由模块，所有端点挂载在 /api 下   │
├─────────────────────────────────────────────────────────────┤
│  服务层 — 16 个业务服务                                       │
│  ├── AIService（核心生成引擎）                                │
│  ├── PipelineService（自动创作流水线，1491 行）               │
│  ├── BookAnalysisService（拆书分析）                          │
│  ├── LlmInvokeService（LLM 统一调用入口）                    │
│  └── ...（记忆/风格/伏笔/节奏/导演等）                       │
├─────────────────────────────────────────────────────────────┤
│  LLM 层 — 9 个文件，支持 12 种提供商                          │
│  ├── providers.ts（提供商配置）                               │
...
│  └── 23 张表：Novel / Chapter / Character / Worldview / ...  │
└─────────────────────────────────────────────────────────────┘
```
...
│   │   └── db/            # Prisma 客户端
│   └── prisma/
│       ├── schema.prisma  # 数据库 Schema（23 张表）
│       └── migrations/    # 6 次迁移
├── shared/                # 共享类型包
│   └── src/types/         # 31 个类型定义文件
├── docs/                  # 项目文档 + 变更记录
└── pnpm-workspace.yaml
```
...
| `/novel/:id` | NovelWorkspace | 作品工作台（13 个标签页） |
...
**核心实体**：Novel → Chapter, Character, Worldview, KnowledgeAsset

**结构化创作**：Volume → ChapterOutline, Mainline, Hook, Foreshadow

**剧情引擎**：StoryState, PleasurePoint, EmotionCurve, ConsistencyIssue

**记忆与风格**：Memory, StyleProfile

**分析与流水线**：BookAnalysis → BookAnalysisSection, ImitationPlan, PipelineJob → PhaseResult

**基础设施**：RagChunk, AppSetting, GenerationLog, GeneralKnowledge, AssetUsageRecord
...
| P1 | 小说主线（Novel/Chapter CRUD + LLM 生成） | ✅ 完成（已超额） |
| P2 | 世界观/人物/资料库 | ✅ 完成（已超额） |
| P3 | 检索增强（RAG） | 待启动 |
