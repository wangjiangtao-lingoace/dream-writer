...
> "墨香流传，笔耕不辍"
 ...
 AI 小说创作平台，支持多种 LLM 提供商（DeepSeek、OpenAI、Qwen、Gemini、MiMo 等 12 种），覆盖从灵感生成到全书自动创作的完整工作流。
...
├─────────────────────────────────────────────────────────────┤
│  服务层 — 58 个业务服务模块 (包括核心、素材与流水线服务)      │
│  ├── AIService（核心生成引擎）                                │
│  ├── PipelineService（自动创作流水线）                        │
│  ├── pipeline/（29 个流水线与剧情阶段处理器）                 │
│  ├── material/（4 个素材分析与拆解服务）                      │
│  └── ...（记忆/风格/伏笔/节奏/导演/导入等 25 个核心服务）     │
├─────────────────────────────────────────────────────────────┤
...
│  └── 40 张表：Novel / Chapter / Character / Worldview / ...  │
└─────────────────────────────────────────────────────────────┘
```
...
│   ├── src/
│   │   ├── routes/        # 31 个路由模块
│   │   ├── services/      # 58 个服务模块 (根目录 25 个)
│   │   │   ├── pipeline/  # 29 个流水线与剧情核心处理器
│   │   │   └── material/  # 4 个素材分析与拆解服务
│   │   ├── llm/           # 9 个 LLM 层核心文件
│   │   └── db/            # Prisma 客户端
│   └── prisma/
│       ├── schema.prisma  # 数据库 Schema（40 张表）
│       └── migrations/    # 8 次迁移
├── shared/                # 共享类型包
...
**结构化创作**：Volume → ChapterOutline, Mainline, Hook, Foreshadow, PayoffChain, ChapterBeat, ChapterSnapshot, CausalLink
...
项目内置了便捷的一键脚本，会自动完成环境自检、安装依赖、同步数据库并拉起浏览器：

```bash
# 1. 启动项目（自动检查依赖并起服）
bash scripts/start.sh

# 2. 停止项目（干净利落地强杀后台占用端口的进程）
bash scripts/stop.sh
```

> **💡 免源码一键分发脚本**
> 
> 在 `scripts/dream-writer.command` 下存有独立分发版启动器。如果您想分享该项目给最终用户，只需**单独拷出**该文件发给他们，用户直接双击运行该脚本，即可在同级目录下自动拉取代码、部署环境并启动，极其简单高效。

### 手动安装步骤

如果您想一步步手动初始化项目，可以参考以下步骤：
...
| P3 | 检索增强（RAG） | ✅ 完成 |
| P4 | 打磨（错误处理/配额/日志/单元测试） | 🟡 部分完成 (错误处理中间件、Jest 32个用例已完成) |
